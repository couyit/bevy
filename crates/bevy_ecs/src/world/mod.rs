...
use error::TryDespawnError;
pub use filtered_resource::*;
pub use identifier::WorldId;
pub use spawn_batch::*;
use unsafe_storage_cell::UnsafeEntityCell;
...
use crate::{
    ...
    component::{
        ChangeTicks, Component, ComponentDescriptor, ComponentHooks, ComponentId, ComponentInfo,
    ...
    resource::Resource,
    resource_components::{ResourceComponents, ResourceDescriptor, ResourceId, ResourceInfo},
    result::Result,
    ...
    world::{
        ...
        error::{EntityFetchError, TryInsertBatchError, TryRunScheduleError},
    },
};
...
pub struct World {
    ...
    pub(crate) resource_components: ResourceComponents,
...
impl World {
    ...
    pub fn register_resource<R: Resource>(&mut self) -> ResourceId {
        self.resource_components.register_resource::<R>()
    }
    ...
    pub fn resource_id<T: Resource>(&self) -> Option<ResourceId> {
        self.resource_components.get_resource_id(TypeId::of::<T>())
    }
    ...
    #[inline]
    #[track_caller]
    pub fn init_resource<R: Resource + FromWorld>(&mut self) -> ResourceId {
        #[cfg(feature = "track_location")]
        let caller = Location::caller();
        let resource_id = self.resource_components.register_resource::<R>();
        if self
            .resources
            .get(resource_id)
    ...
    pub(crate) fn insert_resource_with_caller<R: Resource>(
        ...
        let component_id = self.resource_components.register_resource::<R>();
        OwningPtr::make(value, |ptr| {
            // SAFETY: component_id was just initialized and corresponds to resource of type R.
            unsafe {
                self.insert_resource_by_id(
    ...
    #[inline]
    #[track_caller]
    pub fn init_non_send_resource<R: 'static + FromWorld>(&mut self) -> ResourceId {
        #[cfg(feature = "track_location")]
        let caller = Location::caller();
        let component_id = self.resource_components.register_non_send::<R>();
        if self
            .non_send_resources
            .get(component_id)
            .is_none_or(|data| !data.is_present())
    ...
    pub fn remove_resource<R: Resource>(&mut self) -> Option<R> {
        ...
        let (ptr, _, _) = self.resources.get_mut(component_id)?.remove()?;
    ...
    pub fn remove_non_send_resource<R: 'static>(&mut self) -> Option<R> {
        let component_id = self.components.get_resource_id(TypeId::of::<R>())?;
        let (ptr, _, _) = self.non_send_resources.get_mut(component_id)?.remove()?;
        // SAFETY: `component_id` was gotten via looking up the `R` type
        unsafe { Some(ptr.read::<R>()) }
    }
    ...
    pub fn contains_resource<R: Resource>(&self) -> bool {
        self.components
            ...
            .and_then(|component_id| self.resources.get(component_id))
    ...
    pub fn is_resource_added_by_id(&self, component_id: ComponentId) -> bool {
        self.resources.get(component_id).is_some_and(|resource| {
    ...
    pub fn get_resource_change_ticks<R: Resource>(&self) -> Option<ComponentTicks> {
        self.resource_components
            .get_resource_id(TypeId::of::<R>())
            .and_then(|resource_id| self.get_resource_change_ticks_by_id(resource_id))
    }
    ...
    pub fn get_resource_or_init<R: Resource + FromWorld>(&mut self) -> Mut<'_, R> {
        ...
        let data = unsafe { self.resources.get_mut(component_id).debug_checked_unwrap() };
        // SAFETY: The resource must be present, as we would have inserted it if it was empty.
        let data = unsafe {
            data.get_mut(last_change_tick, change_tick)
                .debug_checked_unwrap()
    ...
    pub fn resource_scope<R: Resource, U>(
        &mut self,
        f: impl FnOnce(&mut SubWorld, Mut<R>) -> U,
    ) -> U {
    ...
    pub fn try_resource_scope<R: Resource, U>(
        ...
        OwningPtr::make(value, |ptr| {
            ...
            unsafe {
                self.resources.get_mut(component_id).map(|info| {
    ...
    pub unsafe fn insert_resource_by_id(
        ...
        let resource = self.initialize_resource_internal(resource_id);
    ...
    pub unsafe fn insert_non_send_by_id(
        ...
        let resource = self.initialize_non_send_internal(resource_id);
        // SAFETY: `value` is valid for `component_id`, ensured by caller
        unsafe {
            resource.insert(
                value,
    ...
    pub(crate) fn initialize_resource_internal(
        ...
        self.resources
            .initialize_with(resource_id, &self.resource_components)
    }
    ...
    pub(crate) fn flush_entities(&mut self) {
        ...
        unsafe {
            self.entities.flush(|entity, location, sub_storage| {
                let empty_archetype = &mut self.archetypes[self.sub_worlds[sub_storage].empty()];
                let table = &mut self.sub_worlds[sub_storage].tables[empty_archetype.table_id()];
    ...
    pub fn check_change_ticks(&mut self) {
        ...
        for storage in self.sub_worlds.storages.iter_mut() {
            storage.tables.check_change_ticks(change_tick);
            storage.sparse_sets.check_change_ticks(change_tick);
        }

        self.resources.check_change_ticks(change_tick);
        self.non_send_resources.check_change_ticks(change_tick);
    ...
    pub fn clear_entities(&mut self) {
        for sub_storage in self.sub_worlds.storages.iter_mut() {
            sub_storage.tables.clear();
            sub_storage.sparse_sets.clear_entities();
        }
...
impl World {
    ...
    /// ```
    #[inline]
    pub fn iter_resources(&self) -> impl Iterator<Item = (&ResourceInfo, Ptr<'_>)> {
        self.resources.iter().filter_map(|(resource_id, data)| {
    ...
    /// ```
    #[inline]
    pub fn iter_resources_mut(&mut self) -> impl Iterator<Item = (&ResourceInfo, MutUntyped<'_>)> {
        self.resources.iter().filter_map(|(resource_id, data)| {
            // SAFETY: If a resource has been initialized, a corresponding ResourceInfo must exist with its ID.
            let component_info = unsafe {
                self.resource_components
                    .get_info(resource_id)
                    .debug_checked_unwrap()
    ...
    /// This function will panic if it isn't called from the same thread that the resource was inserted from.
    #[inline]
    pub fn get_non_send_by_id(&self, resource_id: ResourceInfo) -> Option<Ptr<'_>> {
        // SAFETY:
        // - `as_unsafe_world_cell_readonly` gives permission to access the whole world immutably
        // - `&self` ensures there are no mutable borrows on world data
        unsafe {
            self.as_unsafe_world_cell_readonly()
                .get_non_send_resource_by_id(resource_id)
        }
    }
    ...
    /// This function will panic if it isn't called from the same thread that the resource was inserted from.
    #[inline]
    pub fn get_non_send_mut_by_id(&mut self, resource_id: ResourceInfo) -> Option<MutUntyped<'_>> {
        // SAFETY:
        // - `&mut self` ensures that all accessed data is unaliased
        // - `as_unsafe_world_cell` provides mutable permission to the whole world
        unsafe {
            self.as_unsafe_world_cell()
                .get_non_send_resource_mut_by_id(resource_id)
        }
    }
    ...
    /// use this in cases where the actual types are not known at compile time.**
    pub fn remove_resource_by_id(&mut self, resource_id: ResourceId) -> Option<()> {
        self.resources.get_mut(resource_id)?.remove_and_drop();
        Some(())
    }
    ...
    /// # Panics
    /// This function will panic if it isn't called from the same thread that the resource was inserted from.
    pub fn remove_non_send_by_id(&mut self, resource_id: ResourceId) -> Option<()> {
        self.non_send_resources
            .get_mut(resource_id)?
            .remove_and_drop();
        Some(())
...
pub struct Storage {
    ...
    pub(crate) default_query_filters: Option<DefaultQueryFilters>,
...
impl SubWorlds {
    ...
    pub(crate) fn new() -> Self {
        let mut sub_worlds = Self {
            storages: Vec::with_capacity(1),
            indices: TypeIdMap::default(),
        };

        sub_worlds.create_sub_world::<MainSubWorld>();

        sub_worlds
    }

    pub fn create_sub_world<T: SubWorld>(&mut self) -> SubWorldId {
        let id = SubWorldId(self.storages.len() as u32);
    ...
    pub(crate) fn get_2_mut(
        ...
        if a.as_usize() > b.as_usize() {
            let (b_slice, a_slice) = self.storages.split_at_mut(a.as_usize());
            (&mut a_slice[0], &mut b_slice[b.as_usize()])
        } else {
            let (a_slice, b_slice) = self.storages.split_at_mut(b.as_usize());
...
impl Storage {
    pub fn bootstrap(&mut self) {
        ...
        self.default_query_filters = Some(filters);
    }
    ...
    pub fn entity<F: WorldEntityFetch>(&self, entities: F) -> F::Ref<'_> {
        ...
        #[cold]
        #[track_caller]
        fn panic_no_entity(world: &Storage, entity: Entity) -> ! {
            panic!(
                "Entity {entity} {}",
...
impl IndexMut<SubWorldId> for SubWorlds {
    ...
    fn index_mut(&mut self, index: SubWorldId) -> &mut Self::Output {
        &mut self.storages[index.as_usize()]
    }
}
...
pub trait FromStorage {
    fn from_storage(storage: &mut Storage) -> Self;
}

impl<T: Default> FromStorage for T {
    fn from_storage(_storage: &mut Storage) -> Self {
        T::default()
    }
}
