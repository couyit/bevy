...
impl EventRegistry {
    ...
    pub fn register_event<T: Event>(world: &mut SubWorld) {
    ...
    pub fn run_updates(&mut self, world: &mut SubWorld, last_change_tick: Tick) {
    ...
    pub fn deregister_events<T: Event>(world: &mut SubWorld) {
        let component_id = world.init_resource::<Events<T>>();
        let mut registry = world.get_resource_or_init::<Self>();
        registry
            .event_updates
