...
use crate::{
    entity::Entity,
    system::{entity_command::EntityCommandError, ComponentCommand, EntityCommand},
...
pub trait HandleError<W: ComponentWorld, Out = ()> {
    /// Takes a [`Command`] that returns a Result and uses a given error handler function to convert it into
    /// a [`Command`] that internally handles an error if it occurs and returns `()`.
    fn handle_error_with(self, error_handler: fn(BevyError, ErrorContext)) -> impl ComponentCommand<W>;
    /// Takes a [`Command`] that returns a Result and uses the default error handler function to convert it into
    /// a [`Command`] that internally handles an error if it occurs and returns `()`.
    fn handle_error(self) -> impl ComponentCommand<W>
    where
        Self: Sized,
    {
...
where
    C: ComponentCommand<W, Result<T, E>>,
    E: Into<BevyError>,
    W: ComponentWorld,
{
    fn handle_error_with(self, error_handler: fn(BevyError, ErrorContext)) -> impl ComponentCommand<W> {
...
where
    C: ComponentCommand<W>,
    W: ComponentWorld,
{
    #[inline]
    fn handle_error_with(self, _error_handler: fn(BevyError, ErrorContext)) -> impl ComponentCommand<W> {
        self
    }
    #[inline]
    fn handle_error(self) -> impl ComponentCommand<W>
...
pub trait CommandWithEntity<W: ComponentWorld, Out> {
    /// Passes in a specific entity to an [`EntityCommand`], resulting in a [`Command`] that
    /// internally runs the [`EntityCommand`] on that entity.
    fn with_entity(self, entity: Entity) -> impl ComponentCommand<W, Out> + HandleError<W, Out>;
}
    ...
    W: ComponentWorld,
{
    fn with_entity(
        self,
        entity: Entity,
    ) -> impl ComponentCommand<W, Result<(), EntityMutableFetchError>>
    ...
    W: ComponentWorld,
{
    fn with_entity(
        self,
        entity: Entity,
    ) -> impl ComponentCommand<W, Result<T, EntityCommandError<Err>>>
