...
use super::{CommandQueue, EntityCommands};
...
impl<'w, 's> ParallelCommands<'w, 's> {
    ...
    pub fn command_scope<R>(&self, f: impl FnOnce(EntityCommands) -> R) -> R {
        self.state.thread_queues.scope(|queue| {
            let commands = EntityCommands::new_from_entities(queue, self.entities);
            f(commands)
        })
    }
}
