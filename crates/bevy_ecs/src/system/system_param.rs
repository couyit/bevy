...
unsafe impl<'w, 's, D: ReadOnlyQueryData + 'static, F: QueryFilter + 'static, S: SubWorld>
...
unsafe impl<D: QueryData + 'static, F: QueryFilter + 'static, S: SubWorld> SystemParam
    for Query<'_, '_, D, F, S>
{
...
// SAFETY: Relevant query ComponentId and ArchetypeComponentId access is applied to SystemMeta. If
// this Query conflicts with any prior access, a panic will occur.
unsafe impl<'a, D: QueryData + 'static, F: QueryFilter + 'static, S: SubWorld> SystemParam
    for Single<'a, D, F, S>
{
    type State = QueryState<D, F>;
