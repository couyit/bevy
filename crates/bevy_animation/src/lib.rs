...
impl AnimationEvent {
    fn trigger(&self, commands: &mut EntityCommands, entity: Entity, time: f32, weight: f32) {
...
struct AnimationEventFn(Arc<dyn Fn(&mut EntityCommands, Entity, f32, f32) + Send + Sync>);
...
impl AnimationClip {
    ...
    pub fn add_event(&mut self, time: f32, event: impl Event + Clone) {
        self.add_event_fn(
            time,
            move |commands: &mut EntityCommands, entity: Entity, _time: f32, _weight: f32| {
    ...
    pub fn add_event_fn(
        &mut self,
        time: f32,
        func: impl Fn(&mut EntityCommands, Entity, f32, f32) + Send + Sync + 'static,
    ) {
    ...
    fn add_event_internal(
        ...
        time: f32,
        trigger_fn: impl Fn(&mut EntityCommands, Entity, f32, f32) + Send + Sync + 'static,
    ) {
